...
목표: 반도체 뉴스를 `설계 / 공정 / 소자 / 패키징 / 신제품·발표 / 실적·투자·정책·인사` 여섯 탭으로 분류하되, 원문 전체 복제는 피하고 `제목, 링크, 출처, 날짜, 짧은 요약, 근거 URL` 중심으로 저장한다. 순서는 **수집 → 24시간 날짜 필터 → 키워드 채점**이다. 네이버·Google News·RSS에서 각각 최대 50건씩 받아 실행 시각 기준 24시간 창으로 먼저 거르고, 남은 기사만 `sources.json`의 `sector_keywords` 가중치(3·2·1)로 채점한다. 섹터는 제목 근거를 우선하고, 제목에서 아무 탭도 걸리지 않을 때만 스니펫이 결정한다. 제목 매칭은 2배, 스니펫 매칭은 1배이며, HBM·메모리·반도체처럼 거의 모든 기사에 나오는 단어는 스니펫 신호에서 제외한다. 어느 탭에서도 0점인 기사는 버린다. `신제품·발표`는 새 칩·제품·장비·소프트웨어·SDK·플랫폼·AI 서버·데이터센터 인프라의 출시·공개·제품 로드맵에, `실적·투자·정책·인사`는 실적·영업이익·가이던스·증설·보조금·규제·선임·인사에 사용한다.
...
| 네이버 블로그 검색 | Naver Developers Search API | 커뮤니티 관측 | `category_default`가 community라서 뉴스가 아닌 커뮤니티 풀에서 랭킹·요약된다 |
...
- `반도체 신제품`, `칩 출시`, `SDK`, `컴파일러`, `AI 서버`, `데이터센터 플랫폼`
- `반도체 실적`, `영업이익`, `가이던스`, `증설`, `보조금`, `수출통제`, `인사 선임`
- `반도체 설계`, `팹리스`, `EDA`, `칩렛`, `인터커넥트`, `RTL`
- `첨단 패키징`, `인터포저`, `TSV`, `TC본더`
